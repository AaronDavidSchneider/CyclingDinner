...
def main(file):
    ...
    if c.max_api_calls!=0:
        ...
        winner_comb_geodesic = [total_comb[i] for i in np.argsort(loss_comb_geodesic)]
        if (max_gmaps < len(winner_comb_geodesic)) and (max_gmaps > 0):
            winner_comb_geodesic = winner_comb_geodesic[:max_gmaps]
            loss_comb = [comb.get_loss(gmaps=True) for comb in winner_comb_geodesic]
    ...
    winner_comb.write_emails(np.array(data_sorted).reshape(N_couples),"einladung.txt")
