...
from import_data import food_mapping

inv_food_mapping = {v: k for k, v in food_mapping.items()}
    ...
    def write_emails(self, couples, file):
        ...
        for couple in couples:
            ...
            for c in ["V", "H", "N"]:
                ...
                form_dict["food"] = inv_food_mapping[couple.food]

            for i in range(2):
                form_dict["Notes_" + str(i)] = str(
                    couple.guests[i].note).replace("nan", "-")
                form_dict["G_H_" + str(i)] = couple.guests[i].phone

            out = open(
                "mails/{}_{}.txt".format(slugify(couple.mail),
                                         slugify(couple.name)), "w+")
