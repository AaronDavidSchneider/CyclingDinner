...
class couple:
    ...
    def set_final_combination(self, combination):
        ...
        print(self.name + " has dinner at: {}, {}, {}".format(
            *[self.dinner[c].name for c in ["V", "H", "N"]]))
        print(self.name + " has these guests: {}, {}".format(
            *[self.guests[i].name for i in [0, 1]]))
        print("")
    ...
    def print_map(self):
        ...
        url = f"https://maps.googleapis.com/maps/api/staticmap?zoom=12&scale=2&size=600x600&maptype=roadmap&key={c.API_KEY}&format=png&visual_refresh=true&markers=size:mid%7Ccolor:0xff0000%7Clabel:V%7C{address[0]}&markers=size:mid%7Ccolor:0xff0000%7Clabel:H%7C{address[1]}&markers=size:mid%7Ccolor:0xff0000%7Clabel:N%7C{address[2]}&markers=size:mid%7Ccolor:0x0000FF%7Clabel:CC%7C49.4091357,8.670162"
        wget.download(
            url, "maps/{}_{}.png".format(slugify(self.mail),
                                         slugify(self.name)))
